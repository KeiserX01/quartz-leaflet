...
class FullscreenControl extends SubControl {
    override onAdded(): void {
        if (this.button) {
            this.button.innerHTML = `<iconify-icon icon="lucide:maximize" width="19px" height="19px"></iconify-icon>`;
            this.button.ariaLabel = "Fullscreen";
        }
    }
...
const DefaultControlContainerOptions: ControlContainerOptions = { enableCopyTool: false, enableFullscreen: false };
...
function getControlContainerClass(): ControlContainerClass {
    ...
    class ControlContainer extends L.Control {
        ...
        override onAdd(map: Map): HTMLElement {
            ...
            if (this.settings.enableFullscreen) this.registerSubControl(FullscreenControl, map);
...
async function initialiseMap(
    ...
    const controls = new ControlContainer({
        ...
        enableFullscreen: dataset.enableFullscreen === "true",
    });
